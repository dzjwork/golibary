...
func findWildcard(path string) (wilcard string, i int, valid bool) {
	...
	for start, c := range []byte(path) {
		...
		for end, c := range []byte(path[start+1:]) {
			switch c {
			case '/':
				// 通配符后面遇到的第一个“/”返回通配符信息，通配符起始位置，并且表示通配符有效
				return path[start : start+1+end], start, valid
			case ':', '*':
				// 通配符在遇到“/”之前再次遇到则认为这个信息是无效的
				valid = false
			}
		}
		// 返回通配符到URL结尾的所有字符
...
// 节点结构
type node struct {
	path      string   // 到达节点时经历的字符串路径
	indices   string   // 子节点索引，当子节点为非参数类型，会将每个子节点的首字母放在该索引数组
	wildChild bool     // 子节点中是否包含参数
	nType     nodeType // 节点类型
	priority  uint32   // 优先级
	children  []*node  // 子节点
	handle    Handle   // 该节点对应的处理器
}
